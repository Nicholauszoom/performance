    ...
    <div class="card border-top  border-top-width-3 border-top-main rounded-0">
        <div class="card-body border-0">
            <ul class="nav nav-tabs nav-tabs-underline nav-justified mb-3" id="tabs-target-right" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <a href="#register-approve" class="nav-link" data-bs-toggle="tab" aria-selected="false" role="tab"
                        tabindex="-1">
                        Employee Registered
                    </a>
                </li>
                            ...
                            @if ($row->status == 1 || $row->status >= 5)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <b>FROM: </b> <?= number_format($row->old, 2) ?>/=<br>
                                        <b>TO: </b><?= number_format($row->new, 2) ?>/=
                                        <?php elseif ($row->parameterID == 2): ?>
                                        <?= $flexperformance_model->newPositionTransfer($row->new) ?>
                                        <?php elseif ($row->parameterID == 3): ?>
                                        <b>DEPARTMENT:</b>
                                        <?= $flexperformance_model->newDepartmentTransfer($row->new) ?><br>
                                        <b>POSITION:
                                        </b><?= $flexperformance_model->newPositionTransfer($row->new_position) ?>
                                        <?php elseif ($row->parameterID == 4): ?>
                                        <b>BRANCH:</b>
                                        <?= $flexperformance_model->newBranchTransfer($row->new_department) ?><br>
                                        <b>DEPARTMENT:</b>
                                        <?= $flexperformance_model->newDepartmentTransfer($row->new_department) ?><br>
                                        <b>POSITION:
                                        </b><?= $flexperformance_model->newPositionTransfer($row->new_position) ?>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="options-width">
                                        <a href="<?php echo url('') . '/flex/userprofile/' . $row->empID; ?>" title="Employee Info and Details"
                                        ...
                                        <?php if($row->status==0){ ?>
                                        <a href="javascript:void(0)" onclick="disapproveRequest(<?php echo $row->id; ?>)"
                                            title="Reject" class="icon-2 info-tooltip">
                                        ...
                                        <?php if($row->parameterID==1){
                                        if(session('mng_paym')){  ?>
                                        <a href="javascript:void(0)" onclick="approveSalaryTransfer(<?php echo $row->id; ?>)"
                                            title="Accept" class="icon-2 info-tooltip">
                                            <button type="button" class="btn btn-success btn-xs"><i
                                                    class="fa fa-check"></i></button>
                                        </a>

                                        <?php } } elseif($row->parameterID==2){ ?>
                                        <a href="javascript:void(0)" onclick="approvePositionTransfer(<?php echo $row->id; ?>)"
                                            title="Accept" class="icon-2 info-tooltip">
                                            <button type="button" class="btn btn-success btn-xs"><i
                                                    class="fa fa-check"></i></button>
                                        </a>
                                        ...
                                        <?php }elseif($row->parameterID==3){ ?>
                                        <a href="javascript:void(0)" onclick="approveDeptPosTransfer(<?php echo $row->id; ?>)"
                                            title="Accept" class="icon-2 info-tooltip">
                                            <button type="button" class="btn btn-success btn-xs"><i
                                                    class="fa fa-check"></i></button>
                                        </a>
                                        ...
                                        <?php }elseif($row->parameterID==4){ ?>
                                        <a href="javascript:void(0)" onclick="approveBranchTransfer(<?php echo $row->id; ?>)"
                                            title="Accept" class="icon-2 info-tooltip">
                                            <button type="button" class="btn btn-success btn-xs"><i
                                                    class="fa fa-check"></i></button>
                                        </a>
                        ...
                        <tr>
                            ...
                            <td>
                                <div id ="status<?php echo $row->id; ?>">
                                    <?php if($row->status==5){ ?> <div class="col-md-12"><span
                                            class="badge bg-secondary">WAITING</span></div><?php }
                                        elseif($row->status==6){ ?><div
                                        class="col-md-12"><span class="badge bg-success">ACCEPTED</span></div>
                                    <?php }
                                        elseif($row->status==7){ ?><div class="col-md-12"><span class="badge bg-danger">REJECTED</span>
                                    </div><?php } ?>
                                </div>
                            </td>
                            <td class="options-width">
                                <div class="d-flex">
                                    <a href="<?php echo url('') . '/flex/userprofile/' . base64_encode($row->empID); ?>" title="Employee Info and Details"
                                        class="icon-2 info-tooltip ms-2">
                                        <button type="button" class="btn btn-info btn-xs"><i class="ph-info"></i></button>
                                    </a>
                                    <?php if($row->status==5){ ?>
                                    <a href="javascript:void(0)" onclick="disapproveRegistration(<?php echo $row->id; ?>)"
                                        title="Reject" class="icon-2 info-tooltip ms-2">
                                        <button type="button" class="btn btn-danger btn-xs"><i
                                                class="ph-x"></i></button>
                                    </a>
                                    ...
                                    @if ($level_check == $row->approval_status)
                                        <a href="javascript:void(0)" onclick="approveRegistration(<?php echo $row->id; ?>)"
                                            title="Accept" class="icon-2 info-tooltip ms-2">
                                            <button type="button" class="btn btn-success btn-xs"><i
...
@push('footer-script')
    <script>
        ...
        function approveDeptPosTransfer(id) {
            if (confirm("Are You Sure You Want To Approve This Transfer(The Action may be Irreversible) ?Requirement") ==
                true) {
                var id = id;
        ...
        function disapproveRequest(id) {
            if (confirm(
                "Are You Sure You Want To CANCEL/DELETE This Transfer(The Action may be Irreversible) ?Requirement") == true
                ) {
                var id = id;
                $.ajax({
