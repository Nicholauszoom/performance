    ...
    <div id="approval" class="col-12" tabindex="-1">
        <div class="card border-top  border-top-width-3 border-top-main rounded-0 p-2">
            <div class="modal-content">
                ...
                <form action="{{ route('flex.save-leave-approval') }}" method="POST" class="form-horizontal">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="row mb-3">
                            ...
                            <div class="form-group col-6">
                                ...
                                <select name="empID" class="form-control select">
                                    ...
                                    @foreach ($employees as $item)
                                        <option value="{{ $item->emp_id }}" class="text-center"> {{ $item->fname }}
                            ...
                            <div class="form-group col-6">
                                <label class="col-form-label ">Escallation Time ( <small class="text-danger">*days</small> )
                                </label>
                                <input type="number" required name="escallation_time" placeholder="Enter Escallation Time"
            ...
            @can('add-leave-forfeit')
                <div class="card border-top border-top-width-3 border-top-main rounded-0 p-2">
                    <div class="card-header">
                        <h5 class="text-warning">Employee Leave Approval Mapping </h5>
                    </div>

                    <div class="card-body">
                        <form id="demo-form2" enctype="multipart/form-data" method="post"
                            action="{{ route('flex.upload-leave-approvals') }}" data-parsley-validate
                        ...
                        <a href="{{ asset('uploads/templates/Leave_Approvals_Mapping_Example_Document.xlsx') }}">
                            Click here to download leave approval mapping excel template
                        </a>
                    </div>
        ...
        <div class="card-header border-0">
            <div class="d-flex justify-content-between">
                ...
                <form action="" method="post">
                    ...
                    <button type="submit" class="btn btn-secondary">Download Leave Approval Mapping Excel</button>
                </form>
                        ...
                        <tbody>
                            @foreach ($approvals as $item)
                                <tr>
                                    ...
                                    <td> {{ $item->levelOne->fname ?? '-' }} {{ $item->levelOne->mname ?? '-' }}
                                        {{ $item->levelOne->lname ?? '-' }} </td>
                                    <td>
                                        @if ($item->levelTwo != null)
                                            {{ $item->levelTwo->fname }} {{ $item->levelTwo->mname }}
                                            {{ $item->levelTwo->lname }}
                                        @else
                                            -
                                        @endif
                                    ...
                                    <td>
                                        @if ($item->levelThree != null)
                                            {{ $item->levelThree->fname }} {{ $item->levelThree->mname }}
                                            {{ $item->levelThree->lname }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('flex.editLeaveApproval', $item->id) }}"
                                            class="btn btn-sm bg-main" title="Edit This Leave Approval">
                                            <i class="ph-note-pencil"></i>
                                        </a>
                                        <a href="javascript:void(0)" title="Delete This Approval"
                                            class="icon-2 info-tooltip btn btn-sm btn-danger"
