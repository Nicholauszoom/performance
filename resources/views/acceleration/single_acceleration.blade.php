@extends('layouts.vertical', ['title' => 'View Acceleration'])
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                ...
                <div class="card border-top  border-top-width-3 border-top-main rounded-0">
                  <div class="card-head px-3 py-1">
                    <h2>Programme: {{ $project->name }}       </h2>
                        <small>Start Date: {{ $project->start_date}}</small>
                        <br>
                        <small>End Date: {{ $project->end_date}}</small>
                       
                      <a href="{{ route('flex.acceleration') }}" class="btn btn-perfrom float-end mx-1">
                          <i class="ph-list me-2"></i> All Programmes
                      </a>
                    <a href="{{ url('/flex/add-acceleration-task/'.$project->id) }}" class="btn btn-main float-end">
                        ...
                        <tr>
                            ...
                            <td>
                              ...
                              @if ($item->status==0)
                                ...
                                <a href="{{ url('flex/completed-acceleration-task/'.$item->id); }}" class="btn btn-sm bg-success text-light">
                                  <i class="ph-check"></i>
                              </a>
                              @endif
                              ...
                              @if ($item->created_by==Auth::user()->emp_id)
                                ...
                                <a href="{{ url('flex/delete-acceleration-task/'.$item->id); }}" class="btn btn-sm btn-danger">
                                    <i class="ph-trash"></i>
                                </a> 
                              @endif
                                ...
                                @if ($item->status==1)
                                  ...
                                  <hr>   
                                  <a href="{{ url('flex/assess-acceleration-task/'.$item->id); }}" class="btn btn-sm bg-main">
                                    Assessment
                                  </a> 
                                  @endif
                                @endif
