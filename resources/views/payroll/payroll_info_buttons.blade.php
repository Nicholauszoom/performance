...
<div class="d-flex">
    @if ($payrollState != 0)
        <a
            href="{{ route('payroll.get_reconsiliation_summary', ['payrolldate' => $payroll_date, 'payrollState' => $payroll_state,'type'=>1]) }}">
        ...
        <a class="ms-3"
            href="{{ route('payroll.get_payroll_temp_summary', ['date' => $payroll_date, 'payrollState' => $payrollState, 'type' => 1]) }}"
            target="blank">
        ...
        @endif
    @else
        <a href="{{ route('reports.payrollReconciliationSummary', ['payrolldate' => $payroll_date, 'payrollState' => $payrollState, 'type' => 1]) }}"
            target="">
            <button type="button" name="print" value="print" class="btn btn-main btn-sm"> Reconsiliation</button>
        </a>
        <a class="px-4"
            href="{{ route('reports.payrollReconciliationDetails', ['payrolldate' => $payroll_date, 'payrollState' => $payrollState, 'type' => 1]) }}"
            target="">
            <button type="button" name="print" value="print" class="btn btn-main btn-sm"> Payroll Details</button>
        </a>
