...
<thead style="font-size:8px;">
    <tr class="hdr" style="border-bottom:2px solid rgb(9, 5, 64);">
        ...
        @foreach ($allowance_categories as $row)
            <th class="text-end" style="margin-bottom: 30px;" style="margin-bottom: 30px;"><b>{{ $row->name }}</b>
            </th>
        @endforeach
        ...
        @if ($show_nhif)
            <th class=" {{ $nhifEmployee_col }} text-end nhif-column" style="margin-bottom: 30px;"><b>NHIF Employee</b>
            </th>
            <th class=" {{ $nhifEmployer_col }} text-end nhif-column" style="margin-bottom: 30px;"><b>NHIF Employer</b>
            </th>
...
<tbody>
    ...
    @if ($show_terminations)
       ...
       {{-- {{  dd($row2)  }} --}}
            @php
                $amount2 =
                    $row2->salaryEnrollment +
                    $row2->leaveAllowance +
                    $row2->leavePay -
                    $row2->pension_employee -
                    $row2->paye -
                    $row2->loan_balance;
            @endphp
            ...
            @if ($row2->taxable != 0)
                <tr style="border-bottom:2px solid rgb(67, 67, 73)">
                    ...
                    <td class=" {{ $overtime_col }} text-end">
                        {{ number_format($row2->normal_days_overtime_amount + $row2->public_overtime_amount, 2) }}
                    </td>
                    ...
                    <td class=" {{ $advanceOthers_col }} text-end">
                        {{ number_format($row2->loan_balance + $row2->otherDeductions, 2) }}</td>
                    <td class=" {{ $totalDeduction_col }} text-end">
                        {{ number_format($row2->pension_employee + $row2->paye + $row2->otherDeductions, 2) }}
                    </td>
                    <td class=" {{ $amountPayable_col }} text-end">{{ number_format($amount2, 2) }}</td>
                ...
                @php
                    ...
                    $total_others_term += round(
                        $row2->leavePay +
                            $row2->leaveAllowance +
                            $row2->transport_allowance +
                            $row2->nightshift_allowance +
                            $row2->longServing +
                            $row2->exgracia +
                            $row2->serevancePay +
                            $row2->noticePay +
                            $row2->tellerAllowance,
                        2,
                    );
...
<tfoot>
    <tr style="font-size:10px; !important; border:3px solid rgb(9, 5, 64)">
        ...
        <td class=" {{ $basicSalary_col }} text-end"><b><b>{{ number_format($total_actual_salary, 2) }}</b></b>
        </td>
        ...
        <td class=" {{ $otherPayments_col }} text-end">
            <b><b>{{ number_format($total_others_term + $total_others_normal, 2) }}
                </b></b></td>
        ...
        @if ($show_nhif)
            ...
            <td class=" {{ $nhif_col }} text-end nhif-column">
                <b><b>{{ number_format($totalnhif * 2, 2) }}</b></b></td>
        @endif
