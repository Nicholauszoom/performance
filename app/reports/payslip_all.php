...
// require_once('tcpdf_include.php');
        ...
        foreach($dat[$em->empID]['slipinfo'] as $row){
            $id = $row->empID;
            $old_id = $row->oldID;
        ...
        foreach ($dat[$em->empID]['companyinfo'] as $row) {
            $companyname = $row->cname;
        }

        foreach ($dat[$em->empID]['total_pensions'] as $row) {
            $uptodate_pension_employee = $row->total_pension_employee;
            $uptodate_pension_employer = $row->total_pension_employer;
        }

        $sum_allowances = $dat[$em->empID]['total_allowances'];
        $sum_deductions = $dat[$em->empID]['total_deductions'];
        $sum_loans = 0;
...
// echo $diff->format("%R%a Days");
        $accrued= 7*($diff->format("%R%a"))/90;
        $totalAccrued = (number_format((float)$accrued, 2,'.',''));
        $balance = $totalAccrued - $dat[$em->empID]['annualLeaveSpent'];
...
<table width = "100%">
    <tr>
        ...
        foreach($dat[$em->empID]['allowances'] as $row){
            $allowance  .='
    <tr>
...
//    </tr>';
        ...
        foreach($dat[$em->empID]['deductions'] as $row){
            $deduction  .='
    <tr>
        <td width="500" align="left"><b>'.$row->description.'</b></td>
        ...
        foreach($dat[$em->empID]['loans']as $row){
...
// START TAKE HOME
        ...
        foreach ($dat[$em->empID]['paid_with_arrears'] as $paid_with_arrear){
        ...
        foreach ($dat[$em->empID]['arrears_paid']as $arrear_paid){
        ...
        foreach ($dat[$em->empID]['paid_with_arrears_d'] as $paid_with_arrear_d){
        ...
        if ($with_arr > 0){
            foreach ($dat[$em->empID]['arrears_all'] as $arrear_all){
...
//     </tr>';
//     foreach($dat[$em->empID]['leaves'] as $row){
