...
trait PerformanceModel{
	...
	{
		$query = "COUNT(id) as counts WHERE empID = '".$empID."'";
    ...
    function resource_cost($id) {
        $query = "SUM(cost) as cost WHERE taskID = ".$id."";

        $row =  DB::table('task_resources')
                    ->select(DB::raw($query))
                    ->first();
    ...
    function tasklinemanager_and_staff($id) {
        $query = "SELECT * FROM (SELECT @s:=@s+1 as SNo, t.*, CONCAT(e.fname,' ', e.mname,' ', e.lname) as NAME  FROM task t, employee e, (SELECT @s:=0) as s WHERE e.emp_id = t.assigned_to and t.assigned_by ='".$id."' order BY t.id DESC, SNo ASC)
        AS a UNION ALL SELECT * FROM (SELECT @s:=@s+1 as SNo,  t.*,  CONCAT(e.fname,' ', e.mname,' ', e.lname) as NAME  FROM task t, employee e, (SELECT @s:=0) as s WHERE e.emp_id = t.assigned_to and t.assigned_to ='".$id."' order BY t.id DESC,SNo ASC)
        AS b ORDER BY SNo ASC";
        return DB::select(DB::raw($query));
    }
    ...
    function tasksettings_delay_percent() {
        $query = "value WHERE id =2";

        $row =  DB::table('task_settings')
            ->select(DB::raw($query))
            ->first();
    ...
    function approvedtask($start, $end) {
        $query = "SELECT @s:=@s+1 AS SNo, t.assigned_to AS empID, ROUND(AVG(t.quantity+t.quality),2) AS scores, CONCAT(e.fname,' ', e.mname,' ', e.lname) AS name, d.name AS department, p.name AS position, CONCAT(em.fname,' ', em.mname,' ', em.lname) AS executive,
                    COUNT(t.id) as task_counts, (SELECT tr.id FROM task_ratings tr WHERE tr.lower_limit <= AVG(t.quantity+t.quality)  AND tr.upper_limit>AVG(t.quantity+t.quality)) AS category,
	...
	function outcomeResourceCost($outcomeID) {
		$query = "SUM(tr.cost) as sumCost WHERE tr.taskID = t.id AND t.outcome_ref =  '".$outcomeID."'";

        $row =  DB::table('task_resources as tr', 'outcome o', 'task t')
                    ->select(DB::raw($query))
                    ->first();
